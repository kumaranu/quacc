...
from quacc.recipes.mace.core import _add_stdev_and_hess, freq_job, relax_job
...
def ts_job(
    ...
    **calc_kwargs
    ...
    opt_ts_summary = summarize_opt_run(dyn, additional_fields={"name": "mace_off TS"})
...
def irc_job(
    ...
    **calc_kwargs,
    ...
    freq_job_kwargs
        Keyword arguments to use for the [quacc.recipes.mace.ts.freq_job][]
    opt_params
        Dictionary of custom kwargs for the optimization process. For a list
        of available keys, refer to [quacc.runners.ase.Runner.run_opt][].
    **calc_kwargs
        Custom kwargs for the MACE calculator. Set a value to
        `quacc.Remove` to remove a pre-existing key entirely.
    ...
    with change_settings({"CHECK_CONVERGENCE": False}):
        ...
        opt_irc_summary = summarize_opt_run(
            dyn, additional_fields={"name": f"mace_off IRC: {direction}"}
        )
...
def quasi_irc_job(
    ...
    run_freq
        Whether to run the frequency analysis
    irc_job_kwargs
        Keyword arguments to use for the [quacc.recipes.mace.ts.irc_job][]
    relax_job_kwargs
        Keyword arguments to use for the [quacc.recipes.mace.core.relax_job][]
    freq_job_kwargs
        Keyword arguments to use for the [quacc.recipes.mace.ts.freq_job][]
